...
def text_from_note(note_payload: dict | None) -> tuple[str, str]:
    ...
    def absorb(card: dict) -> None:
        nonlocal title
        for key in ("title", "display_title", "displayTitle"):
    ...
    if isinstance(note_payload.get("items"), list):
        for item in note_payload["items"]:
            if isinstance(item, dict):
                absorb(item)
                card = item.get("note_card") or item.get("noteCard") or item.get("note") or {}
                if isinstance(card, dict):
                    absorb(card)

    absorb(note_payload)
    note_card = (
        note_payload.get("note_card")
        or note_payload.get("noteCard")
...
def iter_comments(comments_payload: dict | list | None) -> list[dict]:
...
def comment_likes(c: dict) -> int:
    for key in ("like_count", "likes", "likeCount"):
        val = c.get(key)
